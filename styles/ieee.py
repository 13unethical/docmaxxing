"""IEEE formatting profile."""

from __future__ import annotations

from styles.academic_spacing import (
    with_heading2_spacing,
    with_heading3_spacing,
    with_title_spacing,
)
from styles.profile import (
    BodyStyleSpec,
    ContextualSpacingRules,
    FontSpec,
    FormattingProfile,
    PageSpec,
    ParagraphFormatSpec,
    ReferencesStyleSpec,
)


def profile() -> FormattingProfile:
    body_font = FontSpec(family="Times New Roman", size_pt=10)

    heading = with_heading2_spacing(
        ParagraphFormatSpec(
            font=FontSpec(family="Times New Roman", size_pt=10, bold=True),
            alignment="left",
            line_spacing=1.0,
            line_spacing_rule="single",
            keep_with_next=True,
            capitalization="none",
        )
    )

    return FormattingProfile(
        id="ieee",
        name="IEEE",
        title=with_title_spacing(
            ParagraphFormatSpec(
                font=FontSpec(family="Times New Roman", size_pt=24, bold=True),
                alignment="center",
                line_spacing=1.0,
                line_spacing_rule="single",
                keep_with_next=True,
            )
        ),
        heading1=heading,
        heading2=heading,
        heading3=with_heading3_spacing(
            ParagraphFormatSpec(
                font=FontSpec(family="Times New Roman", size_pt=10, bold=True),
                alignment="left",
                line_spacing=1.0,
                line_spacing_rule="single",
                keep_with_next=True,
                capitalization="none",
            )
        ),
        body=BodyStyleSpec(
            paragraph=ParagraphFormatSpec(
                font=body_font,
                alignment="justify",
                line_spacing=1.0,
                line_spacing_rule="single",
                first_line_indent_inches=None,
            ),
            contextual=ContextualSpacingRules(
                body_space_before_pt=0,
                body_space_after_pt=6,
                body_space_after_when_next_is_heading_pt=0,
            ),
        ),
        references=ReferencesStyleSpec(
            heading=heading,
            entry=ParagraphFormatSpec(
                font=body_font,
                alignment="left",
                line_spacing=1.0,
                line_spacing_rule="single",
            ),
            contextual=ContextualSpacingRules(
                body_space_after_pt=6,
            ),
        ),
        page=PageSpec(
            margin_top_inches=0.75,
            margin_bottom_inches=1.0,
            margin_left_inches=0.625,
            margin_right_inches=0.625,
        ),
    )
